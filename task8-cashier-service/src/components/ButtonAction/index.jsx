...
const ButtonAction = (props) => {
    return (
        <>
        { props.color === 'orange'
            ? <button onClick={props.action} title={props.title} disabled={props.disabled} className='bg-orange-500 hover:bg-orange-600 text-white text-sm font-semibold w-fit h-fit px-5 py-1.5 rounded-md disabled:cursor-not-allowed disabled:bg-gray-300'>{props.children}</button>
            : props.color === 'green'
            ...
            ? <button onClick={props.action} title={props.title} className='text-orange-500 border border-orange-500 bg-white text-sm font-semibold w-fit h-fit px-5 py-1.5 rounded-md'>{props.children}</button>
            : <></>
        }
        </>
    )
